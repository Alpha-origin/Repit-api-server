...
public class SseEmitterRepository {
    private final ConcurrentMap<String, SseSubscription> emitters = new ConcurrentHashMap<>();

    /**
     * 구독을 등록하고, 같은 작업에 이미 붙어 있던 구독이 있으면 그것을 돌려준다.
     * 밀려난 구독은 흘려보낼 곳이 없으니 호출자가 끊어준다.
     */
    public SseSubscription save(String jobId, SseSubscription emitter) {
        return emitters.put(jobId, emitter);
    }

    public SseSubscription get(String jobId) {
     ...
     */
    public boolean remove(String jobId, SseSubscription emitter) {
     ...
     */
    public void forEach(BiConsumer<String, SseSubscription> action) {
