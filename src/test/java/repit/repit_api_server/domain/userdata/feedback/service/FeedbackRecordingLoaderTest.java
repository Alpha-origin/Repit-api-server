...
class FeedbackRecordingLoaderTest {
    ...
    @Test
    void 채팅_서버_질문_번호를_채점_요청의_질문_id로_옮기고_서명_주소를_싣는다() {
        ...
        List<FeedbackRecording> loaded = loader.load(INTERVIEW_ID, List.of(question(101L, 1L), question(102L, -5L)));
    ...
    @Test
    void 질문을_모르는_영상은_질문_id를_비운다() {
        when(recordingRepository.findAllByInterviewIdOrderByRecordingIdAsc(INTERVIEW_ID)).thenReturn(List.of(
                recording(301L, null), recording(302L, 99L)));

        List<FeedbackRecording> loaded = loader.load(INTERVIEW_ID, List.of(question(101L, 1L)));

        assertThat(loaded).extracting(FeedbackRecording::getQuestionId).containsExactly(null, null);
    }
    ...
    void 영상이_없으면_빈_목록이다() {
        ...
        assertThat(loader.load(INTERVIEW_ID, List.of(question(101L, 1L)))).isEmpty();
    }
